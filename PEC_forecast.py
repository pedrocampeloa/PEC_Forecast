...
os.chdir('D:/Pedro/Desktop/Work/UnB/Tese/Dados')
cwd = os.getcwd()
cwd
...
# models_list = ['RIDGE', 'EN','OLS','ARIMA','RF', 'RW']


def make_forecast(X,y,train_size, forecast_horizon, models_list):
    ...
    for modelname in models_list:
        ...
        while forecast_horizon+i-1<len(X)-train_size:
            ...
            X_treino2_aux = X_treino2.copy()
            X_treino2_aux['Data'] = X_treino2_aux.index
            
            tscv = TimeBasedCV(train_period=len(X_treino2)/5, test_period=len(X_treino2)/10, freq='days')
            index_output = tscv.split(X_treino2.reset_index(), date_column='index')
            
            if modelname=='LASSO':
            
                # print ('Rodando ', modelname)
                # model = LassoCV(cv=5,copy_X=True, fit_intercept=True, verbose=True, normalize=True, alphas = alpha_list)
                ...
                df_coef2[data_pred] = [x for x in model_fit.best_estimator_.fit(X_treino2, y_treino2).coef_] 
                df_alpha = df_alpha.append(pd.DataFrame({'alpha':model_fit.best_estimator_.alpha},index=[data_pred]))   
                ...
                print ('Rodando ', modelname)
                # model = LarsCV(cv=5,copy_X=True, fit_intercept=True, max_iter=1000, verbose=True, normalize=True, epf=alpha_list)
                
                param_grid = {"eps": alpha_list}
                model = RandomizedSearchCV(
                        estimator = Lars(),
                        param_distributions = param_grid,
                        n_iter = 10,
                ...
                df_coef2[data_pred] = [x for x in model_fit.best_estimator_.fit(X_treino2, y_treino2).coef_] 
                df_alpha = df_alpha.append(pd.DataFrame({'alpha':model_fit.best_estimator_.eps},index=[data_pred]))     
                ...
                print ('Rodando ', modelname)
                # model = LassoLarsCV(cv=5,copy_X=True, fit_intercept=True, max_iter=1000, verbose=True, normalize=True, eps = alpha_list)
                ...
                df_coef2[data_pred] = [x for x in model_fit.best_estimator_.fit(X_treino2, y_treino2).coef_[0]] 
                df_alpha = df_alpha.append(pd.DataFrame({'alpha':model_fit.best_estimator_.alpha},index=[data_pred]))    
                ...
                print ('Rodando ', modelname)
                # model = ElasticNetCV(cv=5,copy_X=True, fit_intercept=True, max_iter=1000, verbose=True, normalize=True,
                                       # alphas = alpha_list, l1_ratio = [ratio for ratio in np.arange(0,1,.05)])
                
                
                param_grid = {"alpha": alpha_list, 
                              "l1_ratio":[ratio for ratio in np.arange(0,1,.05)]}
                model = RandomizedSearchCV(
                ...
                df_coef2[data_pred] = [x for x in model_fit.best_estimator_.fit(X_treino2, y_treino2).coef_] 
                df_alpha = df_alpha.append(pd.DataFrame({'alpha':model_fit.best_estimator_.alpha},index=[data_pred]))    
                df_ratio = df_alpha.append(pd.DataFrame({'l1_ratio':model_fit.best_estimator_.l1_ratio},index=[data_pred]))    
                ...
                #                     verbose=True, warm_start=False, n_jobs=-1),
                ...
                # df_coef2[data_pred] = [x for x in model_fit.best_estimator_.fit(X_treino2, y_treino2).coef_] 
                df_max_depth = df_alpha.append(pd.DataFrame({'alpha':model_fit.best_estimator_.max_depth},index=[data_pred]))    
                df_min_samples_split= df_alpha.append(pd.DataFrame({'alpha':model_fit.best_estimator_.min_samples_split},index=[data_pred]))    
                df_n_estimators = df_alpha.append(pd.DataFrame({'alpha':model_fit.best_estimator_.n_estimators},index=[data_pred]))    

        
                n_estimators = model_fit.best_estimator_.n_estimators
                max_depth = model_fit.best_estimator_.max_depth        
                min_samples_split = model_fit.best_estimator_.min_samples_split
                ...
                try:
                    ...
                    coef_dict[modelname]={'coef':df_coef2, 'estimadores':df_n_estimators,'min_samples_split':df_min_samples_split,'max_depth':df_max_depth }
...
def plot_forecast(y, y_treino,y_teste,forecast_horizon, df_metrics):
    ...
    n=4
    fig2, axes = plt.subplots(nrows=n, ncols=1, figsize=(9, 9))
    for i in range(n):
    ...
    plt.savefig('modelo'+str(forecast_horizon)+'dias.eps', format='eps', dpi=500)
    plt.show()
...
def open_results():
    ...
    df_coeff_agg.drop('combined', axis=1).sort_values(['fh', 'var']).to_excel('coefficients_sum.xlsx', index=False, encoding="utf-8")
    df_metrics_agg[df_metrics_agg['model']!='OLS'].drop('Unnamed: 0', axis=1).to_excel('metrics_final.xlsx', index=False, encoding="utf-8")


    models_list = ['RF', 'LASSOLARS', 'LARS','LASSO']
    fig2, axes = plt.subplots(nrows=4, ncols=1, figsize=(8, 10), sharex=True)
    for i,model in enumerate(models_list):
        coef_filter = df_coeff_agg[['var','fh', model]].rename(columns={model:'value'}).sort_values(['fh', 'var'])

        coef_filter_pivotted = coef_filter.pivot(index='fh', columns='var',values='value')

        #NORMALIZE
        # normalized_df=abs(coef_filter_pivotted-coef_filter_pivotted.mean())/coef_filter_pivotted.std()
        if model=='LASSOLARS':
            normalized_df=(coef_filter_pivotted-(coef_filter_pivotted.min()+0.00001))/(coef_filter_pivotted.max()-coef_filter_pivotted.min())
        else:            
            normalized_df=(coef_filter_pivotted-(coef_filter_pivotted.min()))/(coef_filter_pivotted.max()-coef_filter_pivotted.min())        
        stacked_df = normalized_df.divide(normalized_df.sum(axis=1), axis=0)#.fillna(0)
                    

        axes[i].set_title(model)
        axes[i].stackplot(stacked_df.index,[stacked_df[col] for col in stacked_df.columns],
                  labels=stacked_df.columns,alpha=0.8)
        # axe2 = axes[i].twinx()
        # axe2.plot(df_coeff_bm2['fh'],df_coeff_bm2[vars_list[i]], color='orange', label = bm2)
        # axes[i].legend(loc='lower left')
        # axes[i].set_ylabel("coff sum")
        axes[i].set_xticks([1,7,15,30,60,90])
        axes[i].set_ylim([0,1])
        axes[i].set_xlim([1,90])
    
    axes[3].set_xlabel("forecast horizon")   

    fig2.legend(#[l1, l2, l3, l4],     # The line objects
                labels=stacked_df.columns,   # The labels for each line
               loc="center right",   # Position of legend
               borderaxespad=0.1,    # Small spacing around legend box
               title="Variables groups",  # Title for the legend,
               bbox_to_anchor=(1.18, .5)
               )     
    # plt.suptitle(bm+'\n', size=16)
    fig2.tight_layout()

    plt.savefig('stacked_agg.png') 
    plt.show()
    ...
    for i,bm in enumerate(['RF','LASSOLARS', 'EN','RIDGE']):
        ...
        for i in range(n):
            axes[i].set_title(var_dict[vars_list[i]])
            axes[i].plot(df_coeff_bm['fh'],df_coeff_bm[vars_list[i]], label=bm)
            # axe2 = axes[i].twinx()
            # axe2.plot(df_coeff_bm2['fh'],df_coeff_bm2[vars_list[i]], color='orange', label = bm2)
            # axes[i].legend(loc='lower left')
            axes[i].set_ylabel("coff sum")
...
if __name__== "__main3__":
    ...
    X_treino, X_teste,  y_treino, y_teste, df_metrics, coef_dict, df_temp  = make_forecast(X,y,train_size, forecast_horizon, models_list)
    ...
    # n = len(X_treino)
    # for fh in [1,7,15,30,60,90]:
    #         y, X, X_treino, X_teste, y_treino, y_teste, treino, teste = set_database(fh, dados, periodo, train_ratio)
            
    #         df_temp['time_fh{}'.format(fh)] = len(X_treino)/n * df_temp['Time']
            
            
    #         # print('Para o horizonte de previsão {}:\n Base Treino:\n   - data inicial: {} \n   - data final: {}\n Base Teste:\n   - data inicial:{} \n   - data final:{}'.\
    #         #       format(forecast_horizon, dt.datetime.strftime(X_treino.index[0], "%d/%m/%Y"), dt.datetime.strftime(X_treino.index[-1], "%d/%m/%Y"),
    #         #               dt.datetime.strftime(X_teste.index[0], "%d/%m/%Y"), dt.datetime.strftime(X_teste.index[-1], "%d/%m/%Y")))
            
 
    #  df_temp[['time_fh1', 'time_fh7', 'time_fh15', 'time_fh30', 'time_fh60','time_fh90']] = df_temp[['time_fh1', 'time_fh7', 'time_fh15', 'time_fh30', 'time_fh60','time_fh90']].astype(str)
    #  df_temp.to_excel("models_time.xlsx", index=False, encoding="utf-8")
    ...
    cols =  ['mcs',"RMSE","MAE_test","MAPE_test","R2_test"]
    df_metrics[cols[:3]]= df_metrics[cols[:3]].apply(lambda x: round(x,0)).astype(int)
    df_metrics[cols[3:]]= df_metrics[cols[3:]].apply(lambda x: round(x,2)).astype(float)
    
    df_metrics['combined'] = df_metrics[cols].apply(lambda row: '&'.join(row.values.astype(str)), axis=1)
    
    print( df_metrics['model'])
    print(df_metrics['combined'])
...
# for train_index, test_index in tscv.split(X)[-2]:
#     print(train_index)
#     print(test_index)
# #%%
